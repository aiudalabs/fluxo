...
```mermaid
graph TB
  subgraph client["Cliente (agencia / dev-shop)"]
    U["Usuario · Browser"]
    GH["GitHub del cliente<br/>repos · Issues · Actions"]
  end

  subgraph plat["Fluxo · plataforma (AIuda Labs)"]
    C["Console · Next.js<br/>board · studio · agentes · spend · preview"]
    REG["registry/ · el método como data<br/>agents · workflows · skills · templates · providers"]
    ...
    subgraph worker["Worker · Node + Claude Agent SDK (tick 20s)"]
      ...
      PROJ["reconcileProjection<br/>GitHub = verdad → estado"]
      MERGE["reconcileApprovals · AutoMerge (gated)"]
      COST["reconcileCosts · OrphanCosts<br/>(costo, incl. runs cancelados)"]
      ASST["Assistant HTTP :8790 (co-piloto)"]
    end
  end

  SB["Supabase (managed)<br/>Postgres + RLS + Realtime + Auth<br/>brain · stories · sprints · run_costs · design_phases"]

  U -->|HTTPS| C
  U -.->|login GitHub OAuth| GH
  C -->|JWT del tenant · RLS| SB
  C -.->|Realtime| SB
  C -->|▶ despacho manual| GH
  DES -->|Claude Agent SDK| SB
  DISP -->|workflow_dispatch claude.yml<br/>token del cliente · BYO| GH
  GH -->|Action: agente Claude<br/>implementa · commitea · abre PR| GH
  PROJ -->|lee labels / PR / assignee| GH
  MERGE -->|merge squash gated| GH
  COST -->|marcador de costo / log del run| GH
  worker -->|service_role · paths de sistema| SB
  REG -.->|lee el método| worker
